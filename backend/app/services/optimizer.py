from ortools.sat.python import cp_model
 ...
 Hour 56 Simple Optimization Engine
 Step 2.4 Create Basic Optimizer
                    1, 3, f'platform_{i}'
                )
            ...
            for i in range(len(movements)):
                for j in range(i + 1, len(movements)):
                    ...
                    if movement_i.from_station == movement_j.from_station:
                        ...
                        self.model.Add(platform_vars[i] == platform_vars[j]).OnlyEnforceI
                        self.model.Add(platform_vars[i] != platform_vars[j]).OnlyEnforceI
                        ...
                        self.model.Add(departure_vars[i] + 1 <= departure_vars[j]).OnlyEn
                        self.model.Add(departure_vars[j] + 1 <= departure_vars[i]).OnlyEn
                        self.model.Add(or_var1 + or_var2 >= 1).OnlyEnforceIf(same_platfor
            ...
            if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
                ...
                for i, movement in enumerate(movements):
                    ...
                    new_delay = max(0, original_delay + (schedule_diff * 60))  # Convert 
                ...
                improvement = ((total_delay_before - total_delay_after) / max(total_delay
                ...
                return {
                    ...
                    "metrics": {
                        ...
                        "conflicts_resolved": len([m for m in optimized_movements if m["o
                    }
                }
            else:
